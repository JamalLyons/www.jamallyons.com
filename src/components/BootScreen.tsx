...
export function BootScreen({ onBoot }: BootScreenProps) {
	...
	useEffect(() => {
		...
		const chars =
			"01アイウエオカキクケコサシスセソタチツテトナニヌネノハヒフヘホマミムメモヤユヨラリルレロワヲンABCDEFGHIJKLMNOPQRSTUVWXYZ#$%&*()[]{}/+-=<>?~`@0123456789\"'|\\:;.,!?^°±§×÷∞∑µ∂∏∆∇Ωαβγδεζηθικλμνξοπρστυφχψω";
		const fontSize = 14;
	...
	return (
		<div className="w-screen h-screen bg-black relative overflow-hidden">
			...
			{/* Center content */}
			<div
				className={`absolute inset-0 flex items-center justify-center transition-opacity duration-500 p-4 ${isBooting ? "opacity-0" : "opacity-100"}`}
			>
					...
					<div className="space-y-4 px-4">
						...
						<div className="pt-6">
							<button
								...
								className={`font-mono px-6 md:px-8 py-3 border-2 transition-all relative text-sm md:text-base ${
