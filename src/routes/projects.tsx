...
function Projects() {
	...
	const lines = ["[jamal_lyons:~]$ ls ./projects", "", "> Scanning project directory...", ""];
					...
					{showProjects && (
						<div className="space-y-8 pt-4 animate-fade-in">
							{projects.map((project, _index) => (
								<div key={project.name} className="space-y-3 border-l-2 border-accent/30 pl-4">
									<div className="flex items-baseline gap-3">
										<span className="text-terminal font-bold text-lg">&gt; {project.name}/</span>
										<span
									...
									<div className="text-sm text-text/70 pl-2">{project.description}</div>

									<div className="flex flex-wrap gap-2 pl-2">
										{project.tech.map((tech) => (
											<span key={tech} className="text-xs text-accent/80">
												[{tech}]
											</span>
										))}
									</div>
