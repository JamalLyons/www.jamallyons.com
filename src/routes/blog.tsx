...
function Blog() {
	...
	const initLines: Line[] = [
		{
			text: "[jamal_lyons:~]$ ls ./blog",
			style: "text-accent-glow font-bold",
			delay: 100,
		},
		{ text: "", delay: 300 },
		{
			text: "> Scanning blog directory...",
			style: "text-terminal",
			delay: 200,
		},
		{
			text: `> Found ${blogPosts.length} articles`,
			style: "text-terminal",
			delay: 100,
		},
		{ text: "", delay: 500 },
	...
	}, [showContent, isTyping, currentLine, initLines.length, initLines[currentLine]]);
								...
								{blogPosts.map((post, index) => (
									...
									>
										<div className="flex items-baseline gap-3 flex-wrap">
											<span className="text-terminal font-bold text-lg">&gt; {post.title}</span>
											<span className="text-xs text-text/50">
												{post.date} • {post.readTime}
											</span>
										</div>

										<div className="text-sm text-text/70 pl-2">{post.description}</div>
