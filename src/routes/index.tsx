...
export const Route = createFileRoute("/")({
	...
	validateSearch: (search: Record<string, unknown>) => ({
		boot: (search.boot as string) || undefined,
	}),
});
...
function Home() {
	...
	useEffect(() => {
		if (showBootScreen === null) {
			const checkIfBooted = () => {
				...
				const bootedValue = sessionStorage.getItem("boot");
				const bootedTime = sessionStorage.getItem("bootTime");

				if (bootedValue === "1" && bootedTime) {
					const now = Date.now();
					const bootTime = parseInt(bootedTime, 10);
					...
					if (now - bootTime < oneHourInMs) {
						...
						sessionStorage.removeItem("bootTime");
					}
				}
	...
	}, [currentBootLine, bootComplete, showBootScreen]);
	...
	const handleBoot = () => {
		...
		sessionStorage.setItem("bootTime", Date.now().toString());

		// Update URL with query param
		window.history.replaceState(
			{ ...window.history.state, as: undefined, scroll: undefined },
			"",
			`${window.location.pathname}?boot=1`,
		);
						...
						<div className="space-y-2 animate-flicker">
							{bootSequence.slice(0, currentBootLine + 1).map((line, index) => (
								<div
									...
									className={`${index === 0 ? "text-accent-glow font-bold" : "text-terminal"} animate-fade-in`}
								>
									{line}
								</div>
							))}
							{currentBootLine < bootSequence.length && <span className="inline-block animate-blink text-terminal">_</span>}
						</div>
					)}
					...
					{showWelcome && (
						<div className="space-y-8 animate-fade-in">
							<div className="space-y-4 border-l-2 border-accent/50 pl-4 py-4">
								<div className="text-accent-glow text-2xl font-bold">{siteConfig.name}</div>
								<div className="text-text">{siteConfig.role}</div>
								<div className="text-terminal">Building systems that connect people and technology</div>
							</div>
							...
							<div className="space-y-4 pt-4">
								...
								<div className="pl-4 space-y-2 text-text/80">
									...
									<div>
										• Type <span className="text-accent">&gt; about</span> to learn more
									</div>
									<div>
										• Type <span className="text-accent">&gt; projects</span> to see my work
									</div>
								</div>
							</div>
							...
							<div className="space-y-3 pt-6 border-t border-border/30">
								...
								<div className="pl-4 space-y-2 text-sm text-text/70">
									...
									<div>
										Status: <span className="text-terminal">ONLINE</span> | Last Deploy: {new Date().toLocaleDateString()}
									</div>
								</div>
							</div>
